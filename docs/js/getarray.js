...
class GetDataArray {
   ...
   * функции getArray.
   * Для наглядности все вывел в консоль что бы понимать что где и по чем))), поставил таймаут
   * и, наверное это мне и дало возможность как то разобраться немного в ситуации. Но только от части.
   * Точнее я понимаю, что при том способе записи
   * что применяю я функция от чего-то не возвращает то что мне надо, но как сделать правильно я не понимаю
   * сломал весь мозг)))
  ...
  getArray = () => {
    ...
    if (this.arrayUrlNum === 0) {
      ...
      /**Короче что то начинаю понимать. Но уяснить как это сделать правильно пока не могу
       * Я понял, что я как то не так возвращаю результат работы функции это первое и второе
       * это то, что я не разобрался с асинхронностью. Точнее так. Поставив Таймаут я добился
       * вывода в консоль нужного результата, но думаю что это не совсем правильно.
       * Так что надо разобраться с асинхронностью и с выводом функции нужного результата.
       */
